...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public void updatePasswordByPhone(String cellPhone, String code, String password) {
        ...
        // 新增：删除旧 refreshToken
        // ======================
        deleteUserRefreshToken(oldUser.getId());
    }
    ...
    public void updateUserStatus(Long userId, Integer status) {
        ...
        // 判断用户是否存在
        AssertUtils.isTrue(success, UserErrorInfo.Msg.USER_ID_NOT_EXISTS);

        // 2. 如果是【禁用】操作 → 删除 Redis 中的 refreshToken，强制让用户重新登录
        if (UserStatus.FROZEN == UserStatus.of(status)) {
            deleteRefreshToken(userId);
        }
    }

    /**
     * 通用方法：删除用户的所有 refreshToken
     */
    private void deleteRefreshToken(Long userId) {
        // 学生端/管理端 都删掉
        stringRedisTemplate.delete("refresh:user:" + userId);
        stringRedisTemplate.delete("refresh:admin:" + userId);
    }
    ...
    /**
     * 删除用户的 refreshToken，让旧登录失效
     */
    private void deleteUserRefreshToken(Long userId) {
        // 对应 auth 模块里的 Redis key
        String refreshTokenKey = "refresh:user:" + userId;
        String adminRefreshTokenKey = "refresh:admin:" + userId;

        stringRedisTemplate.delete(refreshTokenKey);
        stringRedisTemplate.delete(adminRefreshTokenKey);
    }
}
