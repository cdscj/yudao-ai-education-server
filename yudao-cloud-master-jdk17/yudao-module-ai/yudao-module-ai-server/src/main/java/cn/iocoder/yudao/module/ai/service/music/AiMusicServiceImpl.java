...
public class AiMusicServiceImpl implements AiMusicService {
    ...
    public List<Long> generateMusic(Long userId, AiSunoGenerateReqVO reqVO) {
        // 1. 创建待生成记录
        List<AiMusicDO> musicList = new ArrayList<>();
        for (int i = 0; i < 2; i++) {
            AiMusicDO music = new AiMusicDO().setUserId(userId)
                    .setDescription(reqVO.getPrompt()).setPlatform(reqVO.getPlatform())
                    .setGenerateMode(reqVO.getGenerateMode()).setTitle(reqVO.getTitle())
                    .setStatus(AiMusicStatusEnum.IN_PROGRESS.getStatus());
            musicMapper.insert(music);
            musicList.add(music);
        }

        // 2. 异步生成音乐
        getSelf().executeGenerateMusic(musicList, reqVO);
        return convertList(musicList, AiMusicDO::getId);
    }

    @Async(AI_THREAD_POOL_TASK_EXECUTOR)
    public void executeGenerateMusic(List<AiMusicDO> musicList, AiSunoGenerateReqVO reqVO) {
        try {
            ...
            if (CollUtil.isNotEmpty(musicDataList)) {
                List<AiMusicDO> updateList = buildMusicDOList(musicDataList);
                for (int i = 0; i < updateList.size() && i < musicList.size(); i++) {
                    updateList.get(i).setId(musicList.get(i).getId());
                }
                musicMapper.updateBatch(updateList);
            }
        } catch (Exception ex) {
            log.error("[executeGenerateMusic][music 生成异常]", ex);
            musicList.forEach(music -> musicMapper.updateById(
                    new AiMusicDO().setId(music.getId()).setStatus(AiMusicStatusEnum.FAIL.getStatus())
                            .setErrorMessage(ex.getMessage())));
        }
    }

    private AiMusicServiceImpl getSelf() {
