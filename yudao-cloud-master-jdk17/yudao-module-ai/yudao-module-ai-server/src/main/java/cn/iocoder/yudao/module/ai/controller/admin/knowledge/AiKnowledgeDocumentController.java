...
public class AiKnowledgeDocumentController {
    ...
    @PostMapping("/upload")
    @Operation(summary = "上传文档（支持 PDF、Word、Markdown 等格式）")
    @PreAuthorize("@ss.hasPermission('ai:knowledge:create')")
    public CommonResult<Long> uploadKnowledgeDocument(@RequestParam("knowledgeId") Long knowledgeId,
                                                     @RequestParam("file") MultipartFile file,
                                                     @RequestParam(value = "name", required = false) String name,
                                                     @RequestParam("segmentMaxTokens") Integer segmentMaxTokens) {
        AiKnowledgeDocumentUploadReqVO uploadReqVO = new AiKnowledgeDocumentUploadReqVO();
        uploadReqVO.setKnowledgeId(knowledgeId);
        uploadReqVO.setFile(file);
        uploadReqVO.setName(name);
        uploadReqVO.setSegmentMaxTokens(segmentMaxTokens);
        Long id = documentService.createKnowledgeDocumentByFile(uploadReqVO);
        return success(id);
    }
