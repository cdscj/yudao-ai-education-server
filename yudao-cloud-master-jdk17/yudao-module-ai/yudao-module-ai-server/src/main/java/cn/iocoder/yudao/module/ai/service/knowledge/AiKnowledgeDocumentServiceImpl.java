...
import cn.iocoder.yudao.module.ai.controller.admin.knowledge.vo.document.AiKnowledgeDocumentUploadReqVO;
...
public class AiKnowledgeDocumentServiceImpl implements AiKnowledgeDocumentService {
    ...
    public Long createKnowledgeDocumentByFile(AiKnowledgeDocumentUploadReqVO uploadReqVO) {
        ...
        String fileName = uploadReqVO.getName();
        if (StrUtil.isEmpty(fileName)) {
            fileName = file.getOriginalFilename();
        }
        String content = readMultipartFile(file);

        // 3. 文档记录入库
        AiKnowledgeDocumentDO documentDO = new AiKnowledgeDocumentDO()
                .setKnowledgeId(uploadReqVO.getKnowledgeId())
                .setName(fileName)
                .setContent(content)
                .setContentLength(content.length())
                .setTokens(tokenCountEstimator.estimate(content))
                .setSegmentMaxTokens(uploadReqVO.getSegmentMaxTokens())
                .setStatus(CommonStatusEnum.ENABLE.getStatus());
        knowledgeDocumentMapper.insert(documentDO);
    ...
    private String readMultipartFile(MultipartFile file) {
        try {
            ...
            return readFileContent(new ByteArrayResource(bytes));
        } catch (Exception e) {
            log.error("[readMultipartFile][file({}) 读取失败]", file.getOriginalFilename(), e);
            throw exception(KNOWLEDGE_DOCUMENT_FILE_UPLOAD_FAIL);
        }
    }

    private String readFileContent(ByteArrayResource resource) {
