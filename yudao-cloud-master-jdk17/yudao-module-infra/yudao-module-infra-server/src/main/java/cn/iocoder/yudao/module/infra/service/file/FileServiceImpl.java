...
import cn.iocoder.yudao.module.infra.framework.file.config.YudaoFileProperties;
...
public class FileServiceImpl implements FileService {
    ...
    @Resource
    private YudaoFileProperties yudaoFileProperties;

    private volatile FileClient yamlFileClient;

    private FileClient getFileClient() {
        if (yudaoFileProperties != null && yudaoFileProperties.getAliyun() != null) {
            if (yamlFileClient == null) {
                synchronized (this) {
                    if (yamlFileClient == null) {
                        yamlFileClient = createYamlFileClient();
                    }
                }
            }
            return yamlFileClient;
        }
        return fileConfigService.getMasterFileClient();
    }

    private FileClient createYamlFileClient() {
        YudaoFileProperties.Aliyun aliyun = yudaoFileProperties.getAliyun();
        S3FileClientConfig config = new S3FileClientConfig();
        config.setEndpoint(aliyun.getEndpoint());
        config.setAccessKey(aliyun.getAccessKeyId());
        config.setAccessSecret(aliyun.getAccessKeySecret());
        config.setBucket(aliyun.getBucketName());
        config.setDomain(aliyun.getDomain());
